...
const ContentManager = () => {
    ...
    const [categoryTreeRoots, setCategoryTreeRoots] = useState([]);
    ...
    const selectedHeader = useMemo(
        () => categoryTreeRoots.find(h => h._id === selectedHeaderId) || null,
        [categoryTreeRoots, selectedHeaderId]
    );

    const loadCategoryTreeRoots = async () => {
        try {
            const res = await adminApi.getCategoryTree();
            if (res.data.success) {
                const tree = res.data.results || res.data.result || [];
                const roots = Array.isArray(tree) ? tree : [];
                setCategoryTreeRoots(roots);
                if (!selectedHeaderId && roots.length) {
                    setSelectedHeaderId(roots[0]._id);
                }
            }
        } catch (e) {
            console.error(e);
            showToast('Failed to load parent categories', 'error');
        }
    };
    ...
    const handleSaveSection = async () => {
        ...
        if (displayType === 'banners') {
            ...
            const categoryIdsForSubs =
                pageType === 'header' && selectedHeaderId
                    ? [selectedHeaderId]
                    : formData.subCategoryCategoryIds;
            if (!categoryIdsForSubs?.length || !formData.subCategoryIds?.length) {
                showToast('Please select categories and subcategories', 'warning');
                return;
            }
            config = {
                categoryIds: categoryIdsForSubs,
            ...
            <div className="flex flex-wrap gap-4 items-center mt-6">
                ...
                {pageType === 'header' && (
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Page anchor</span>
                        <select
                    ...
                    {formData.displayType === 'subcategories' && (
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(pageType === 'home' ? categoryTreeRoots : selectedHeader ? [selectedHeader] : []).map(c => {
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(pageType === 'header' && selectedHeader
