...
const CategoryManagement = () => {
    ...
    const filteredCategories = useMemo(() => {
        ...
        if (!debouncedSearchTerm && filterStatus === 'all') return categories;
    ...
    const allSubcategories = useMemo(() => {
        const subs = [];
        const traverse = (items, parentCategoryName = '') => {
            items.forEach(item => {
                if (item.type === 'category' && (!item.parentId || item.parentId === null)) {
    ...
    }, [allSubcategories, debouncedSearchTerm, filterStatus]);
    ...
    const handleSave = async () => {
        ...
        const nextParentId = formData.type === 'category' ? '' : String(formData.parentId || '');

        if (!trimmedName) return toast.error('Name is required');
        if (!nextSlug) return toast.error('Please enter a valid name');
        if (formData.type === 'subcategory' && !nextParentId) return toast.error('Parent is required');
    ...
    const openModal = (type, parentId = '', item = null) => {
        if (item) {
            setFormData({
                ...
                type: item.type,
                parentId: item.type === 'category' ? '' : (item.parentId || ''),
                order: item.order || 0
            });
            setEditingItem(item);
                ...
                <div className="xl:col-span-4 space-y-6">
                    <Card className="bg-slate-900 border-none shadow-xl p-6 rounded-xl text-white relative overflow-hidden">
                        <div className="relative z-10">
                            ...
                            <div className="space-y-4">
                                    {[
                                    { step: '1', title: 'Parent categories', desc: 'Top-level groups (e.g. Grocery).', color: 'bg-indigo-500' },
                                    { step: '2', title: 'Subcategories', desc: 'Specific shelves under each parent (e.g. Milk).', color: 'bg-amber-500' }
                                ].map((item, i) => (
                                    ...
                                    {formData.type !== 'category' && (
                                        <div className="space-y-1">
                                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">
                                                Parent {formData.type === 'subcategory' ? 'category' : ''} <span className="text-rose-500">*</span>
                                            </label>
                                                    ...
                                                    {parentUnits
                                                        .filter(unit => {
                                                            if (formData.type === 'subcategory') {
                                                                return unit.type === 'category' && !(unit.parentId?._id || unit.parentId);
                                                            }
                                                            return false;
                                                        })
                                                        .map(unit => (
