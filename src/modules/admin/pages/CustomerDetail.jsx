...
const CustomerDetail = () => {
    ...
    const handleRestrictAccount = async () => {
        ...
        try {
            const { data } = await adminApi.updateUserStatus(id, { isActive: !willSuspend });
            if (data.success) {
                const nextStatus = data.result?.status || (willSuspend ? 'inactive' : 'active');
                setCustomer({ ...customer, status: nextStatus });
                setIsRestrictModalOpen(false);
                showToast(
                    willSuspend ? 'Customer suspended — they cannot use the app' : 'Customer account activated',
                        ...
                        <div className="relative shrink-0">
                            <img
                                ...
                                onError={(e) => {
                                    ...
                                    e.target.src = getCustomerAvatarUrl({ ...customer, avatar: '' });
                                }}
                            />
                            <div className={cn(
                    ...
                    <div className="p-6 bg-rose-50 rounded-xl border border-rose-100 flex flex-col items-center text-center gap-4">
                        ...
                        <p className="text-sm font-bold text-slate-500 leading-relaxed">
                            {customer.status === 'active'
                                ? 'The customer cannot log in or use the app. COD block is separate and does not stop login.'
                                : 'The customer can log in and use the app again.'}
                        </p>
                    </div>
